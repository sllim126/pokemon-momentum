...
INDICATORS = Path(r"F:\Pokemon historical data extracted\top200_indicators.csv")
LOOKUP     = Path(r"F:\Pokemon historical data extracted\top200_lookup.csv")
OUT_HTML   = Path(r"F:\Pokemon historical data extracted\top200_dashboard.html")
...
keys = df[["productId", "subTypeName", "productName", "groupName"]].drop_duplicates()
...
for _, k in keys.iterrows():
    ...
    series_map[f"{pid}::{st}"] = {
        ...
        "subTypeName": st,
        "label": label,
        "groupName": "" if pd.isna(k["groupName"]) else str(k["groupName"]),
